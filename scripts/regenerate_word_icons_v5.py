#!/usr/bin/env python3
"""Regenerate specific word icons - round 5. Object-focused hints."""
...
GRIDS_DIR = OUTPUT_DIR / "grids-redo5"

# Simple objects only - no actions or abstract concepts
WORDS_V5 = {
    'snap': 'ginger snap cookie',
    'skip': 'jump rope with handles',
    'fell': 'bandaid on scraped knee',
    'this': 'big red X marks the spot',
    'that': 'pointing hand emoji',
    'then': 'clock with arrow',
    'out': 'open cardboard box empty',
    'show': 'movie theater stage with curtains',
    'brown': 'brown crayon',
    'will': 'strong arm muscle flexing',
    'day': 'smiling yellow sun face',
    'for': 'wrapped birthday present with bow',
    'mop': 'string mop with wooden handle',
    'well': 'wishing well with stone walls and roof',
}

WORDS_TO_REDO = list(WORDS_V5.keys())
...
def generate_grid(batch_num: int, words: list, config) -> bool:
    grid_path = GRIDS_DIR / f"redo5_grid_{batch_num:02d}.png"

    panels = []
    for i, word in enumerate(words[:9]):
        hint = WORDS_V5.get(word, word)
        panels.append(f"Panel {i+1}: {hint}")

    while len(panels) < 9:
        panels.append(f"Panel {len(panels)+1}: cute star")

    prompt = f"""9 simple cartoon icons in a 3x3 grid on pure WHITE background.
...
- Each icon TINY (40% of cell) and CENTERED with lots of white space
- Kawaii style, bold black outlines, bright colors
- NO text or letters
- Nothing cut off - complete icons only
...
def main():
    print(f"Regenerating {len(WORDS_TO_REDO)} icons (round 5)\n")
